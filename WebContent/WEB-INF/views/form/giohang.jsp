...
<body>
    <div class="app">
        ...
        <div class="app-container">
            <div class="grid">
				<div class="grid--row">
					<div class="grid--column-2-5-1">
						<h3 class="cart--list--header">HÌNH ẢNH</h3>
                    </div>
                    <div class="grid--column-2-5-2">
						<h3 class="cart--list--header">TÊN SẢN PHẨM</h3>
						
					</div>

					<div class="grid--column-2-5">
						<h3 class="cart--list--header">SỐ LƯỢNG</h3>
						
					</div>

					<div class="grid--column-2-5">
						<h3 class="cart--list--header">ĐƠN GIÁ</h3>
						
					</div>

					<div class="grid--column-2-5">
						<h3 class="cart--list--header">THÀNH TIỀN</h3>
						
					</div>

					<div class="grid--column-2-5-1">
						<h3 class="cart--list--header">TÁC VỤ</h3>
					</div>
                </div>
                <c:forEach var = "ctgh" items = "${ctghs }" varStatus = "status">
                <div class="grid--row grid--row1">
					<div class="grid--column-2-5-1 cart--list--contain">
						<img src="${ctgh.getValue().getS().getLink() }" alt="" class="cart--list-img">
                    </div>
                    <div class="grid--column-2-5-2 cart--list--contain">
						<span class="cart--list--contain-label">${ctgh.getValue().getS().getTen() }</span>
						
					</div>

					<div class="grid--column-2-5 cart--list--contain">
                        <div class="cart--list--contain-amout">
                            
                            <c:if test="${ctgh.getValue().getSoluong() != 1 }">
                                 <a href = "home/giohang/update/${ctgh.getKey()}/1.htm" class= "home--detail--contain-amount-link">
                                     <i class="fas fa-minus-circle cart--list--contain-icon"></i>
                                  </a>
                           </c:if>
                           <c:if test="${ctgh.getValue().getSoluong() == 1 }">
                               <i class="fas fa-minus-circle cart--list--contain-icon"></i>
                           </c:if>
						<span class="cart--list--contain-label">${ctgh.getValue().getSoluong() }</span>
							<a href = "home/giohang/update/${ctgh.getKey()}/2.htm" class= "home--detail--contain-amount-link">
								<i class="fas fa-plus-circle cart--list--contain-icon"></i>
							</a>
                        </div>
					</div>

					<div class="grid--column-2-5 cart--list--contain">
						<span class="cart--list--contain-label">
						<fmt:formatNumber value="${ctgh.getValue().getS().getGia() * ((100 - ctgh.getValue().getS().getGiamGia()) / 100) }" type="currency" pattern = "###,###,### đ"/>
						</span>
					</div>
					<div class="grid--column-2-5 cart--list--contain">
						<span class="cart--list--contain-label">
						<fmt:formatNumber value="${ctgh.getValue().getS().getGia() * ((100 - ctgh.getValue().getS().getGiamGia()) / 100) * ctgh.getValue().getSoluong()}" type="currency" pattern = "###,###,### đ"/>
						</span>
					</div>

					<div class="grid--column-2-5-1 cart--list--contain">
						<button class="cart--list--contain-label"><a class = "cart--list--contain-link" href="home/giohang/update/${ctgh.getKey()}/3.htm">Xóa</a></button>
					</div>
                </div>
                </c:forEach>
                <div class="grid--row">
					...
					<div class="grid--column-2-5 cart--list--contain--info">
						<h3 class="cart--list--contain-label">TỔNG TIỀN :</h3>
					</div>

					<div class="grid--column-2-5 cart--list--contain--info">
						<h3 class="cart--list--contain-label">
						<fmt:formatNumber value="${tongtien }" type="currency" pattern = "###,###,### đ"/>
						</h3>
					</div>

					<div class="grid--column-2-5-1 cart--list--contain--info">
						<button class="btn--cart--list"><a class = "btn--cart--list-link" href="home/dathang/${tongtien}/tien.htm">ĐẶT HÀNG</a></button>
					</div>
                </div>
			</div>
        </div>
